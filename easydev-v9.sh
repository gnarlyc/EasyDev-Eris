...
# used code from here to check java version --> http://notepad2.blogspot.com/2011/05/bash-script-to-check-java-version.html 
...
apt-get update
apt-get install git-core gnupg flex bison gperf libsdl-dev libesd0-dev libwxgtk2.6-dev squashfs-tools build-essential zip
...
if [ $VER >= 6 ]
   then
      ...
      echo ""
      echo "Java 1.6+ did not get installed. Please check. ENTER to continue"
	  read -p 
	  echo ""
fi
